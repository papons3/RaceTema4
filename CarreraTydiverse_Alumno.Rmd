...
```{r}
arrange(cran2, country,ip_id, desc(r_version))
```
...
```{r}
cran3 %>% mutate(size_gb = size /2^10)
```
...
```{r}
summarise(by_package, avg_size = mean(size))
...
```{r}
pack_sum<- by_package %>% summarise(count = n(),
                                    unique = n_distinct(ip_id),
                                    countries = n_distinct(country),
                                    med_bytes =median(size, na.rm = TRUE))
pack_sum
...
top_counts<- cran %>% 
  group_by(package)%>%
  summarise(descargas = n())%>%
  filter(descargas > quantile(descargas, probs=0.99))
top_counts
```
...
top_counts<- cran %>% 
  group_by(package)%>%
  summarise(count = n())%>%
  filter(count > quantile(count, probs=0.99)) %>%
  arrange(desc(count))
top_counts
